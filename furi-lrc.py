...
/* inline-block breaks ruby positioning; base unit must stay inline */
ruby > span.unit { display: inline; }
/* Karaoke sweep via CSS gradient + background-clip */
.unit {
...
function setTheme(sung, unsung, zh, fontJP, fontZH, fsizeJP, fsizeZH, spacingZH) {
...
function buildJP(parent, li, segs) {
  ...
  segs.forEach((seg, si) => {
    if (seg.ruby) {
      /* Has furigana: base char swept proportionally, each rt mora swept individually */
      ...
      /* No ruby: each unit is a plain inline span */
      seg.units.forEach((u, ui) => {
        const s = document.createElement('span');
        s.className = 'unit';
        s.id = 'U' + li + '_' + si + '_' + ui;
...
function sweep() {
  if (lyrics.length === 0) { requestAnimationFrame(sweep); return; }

  const ms = nowMs();
  const li = findLine(ms);

  /* Line-level highlight — only one line visible at a time */
  if (li !== activeI) {
    if (activeI >= 0) {
  ...
  /* Unit-level karaoke — only process ±1 line around active for perf */
  const lo = Math.max(0, li - 1);
  const hi = Math.min(lyrics.length - 1, li + 2);
  for (let i = lo; i <= hi; i++) updateUnits(i, ms);

  requestAnimationFrame(sweep);
}

function updateUnits(li, ms) {
  lyrics[li].jp.forEach((seg, si) => {
    const n = seg.units.length;
    if (!n) return;
    if (seg.ruby) {
      let sum = 0;
      seg.units.forEach((u, ui) => {
        const p = clamp01((ms - u.s) / ((u.e - u.s) || 1));
        const el = document.getElementById('R' + li + '_' + si + '_' + ui);
        if (el) el.style.setProperty('--p', p);
        sum += p;
      });
      /* Base char fill = average mora progress */
      const bel = document.getElementById('B' + li + '_' + si);
      if (bel) bel.style.setProperty('--p', clamp01(sum / n));
    } else {
      seg.units.forEach((u, ui) => {
        const p = clamp01((ms - u.s) / ((u.e - u.s) || 1));
        const el = document.getElementById('U' + li + '_' + si + '_' + ui);
        if (el) el.style.setProperty('--p', p);
      });
    }
  });
...
class SettingsDialog(QDialog):
    ...
    def _build(self):
        ...
        f1.addRow("中文フォント",         self.font_zh_w)
        f1.addRow("日本語フォントサイズ", self.font_size_jp)
        f1.addRow("中文フォントサイズ",   self.font_size_zh)
...
class LyricWindow(QWidget):
    ...
    def _apply_theme_cfg(self, cfg: dict):
        ...
        js = (
            ...
            f"{c['font_size_jp']},{c['font_size_zh']},{c['spacing_zh']});"
        )
